...
class KeyboardListener:
    def __init__(self, root):
        ...
        self.current_line_keys = []  # 当前行的按键序列
    ...
    def on_press(self, key):
        try:
            ...
            if time_diff > self.settings.settings["combination_timeout"] and self.current_line_keys:
                self.add_to_history(" + ".join(filter(None, self.current_line_keys)))
                self.current_line_keys = []
            
            # 添加新按键到当前行
            if key_name not in self.current_line_keys:
    ...
    def update_display(self):
        try:
            ...
            # 如果超过超时时间，将当前行添加到历史
            if time_diff > self.settings.settings["combination_timeout"] and self.current_line_keys:
                # 过滤掉 None 值并只保留有效的字符串
                valid_keys = [k for k in self.current_line_keys if k is not None and isinstance(k, str)]
                if valid_keys:
                    self.add_to_history(" + ".join(valid_keys))
                self.current_line_keys = []
            ...
            for i, label in enumerate(self.key_labels):
                if i == 0:  # 第一行显示当前正在输入的内容
                    # 过滤掉 None 值并只保留有效的字符串
                    valid_keys = [k for k in self.current_line_keys if k is not None and isinstance(k, str)]
                    current_text = " + ".join(valid_keys) if valid_keys else ""
