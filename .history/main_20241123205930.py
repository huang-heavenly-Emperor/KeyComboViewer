...
class KeyboardListener:
    def __init__(self, root):
        try:
            ...
            self.current_combination = set()  # 当前正在收集的组合键
            self.last_key_time = datetime.now()  # 最后一次按键时间
            self.pending_combination = None  # 待确认的组合键
    ...
    def setup_ui(self):
        try:
            ...
            tk.messagebox.showerror("UI错误", f"UI初始化失败: {str(e)}")
            raise
    ...
    def on_press(self, key):
        ...
        if time_diff > self.settings.settings["combination_timeout"]:
            if self.pending_combination:
                self.add_to_history(self.pending_combination)
            self.current_combination.clear()
        
        # 添加当前按键到组合键中
        self.current_combination.add(key_name)
        self.last_key_time = current_time
        
        # 更新待确认的组合键
        if self.current_combination:
            self.pending_combination = " + ".join(sorted(self.current_combination))
    ...
    def update_display(self):
        ...
        # 如果超过超时时间且有待确认的组合键，添加到历史记录
        if time_diff > self.settings.settings["combination_timeout"] and self.pending_combination:
            self.add_to_history(self.pending_combination)
            self.current_combination.clear()
